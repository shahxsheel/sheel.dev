# later.md — cut from v1, drop in without a rewrite

Each line is one deferred thing. Do not build these until v1 is shipped and live.

- Binder open animation — same `.object` element becomes an animated layer (DESIGN.md §5.1).
- Magazine page-flip animation on the work hotspot.
- Parallax on the stage as the cursor moves.
...
- Markdown → HTML upgrade if project bodies ever need headings/lists (currently paragraphs only).
